...
void BasSimulateBattery(cy_stc_ble_conn_handle_t connHandle)
{
    ...
    if(--batteryTimer == 0u)
    {
        ...
        apiResult = Cy_BLE_BASS_GetCharacteristicDescriptor(connHandle, CY_BLE_BAS_BATTERY_LEVEL, CY_BLE_BAS_BATTERY_LEVEL, CY_BLE_BAS_BATTERY_LEVEL_CCCD, CY_BLE_CCCD_LEN, (uint8_t*)&cccd);
        
        if(apiResult != CY_BLE_SUCCESS)
        {
            DBG_PRINTF("Cy_BLE_BASS_GetCharacteristicDescriptor API Error: 0x%x \r\n", apiResult);
        }
