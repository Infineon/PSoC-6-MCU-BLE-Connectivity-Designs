...
void ProcessUartCommands(char8 command)
{
    ...
    switch(command)
    {
        case 'g': /* Send HTTP GET request to read LED state on HTTP Proxy Server */
            HpsSendRequest(CY_BLE_HPS_HTTP_GET, 0u);
            DBG_PRINTF("An HTTP 'GET' request was sent \r\n");
            break;
            
        case '0': /* Send HTTP POST request to clear LED state on HTTP Proxy Server */
            HpsSendRequest(CY_BLE_HPS_HTTP_POST, 0u);
            DBG_PRINTF("An HTTP 'POST' request with parameter 'LED OFF' was sent \r\n");
            break;
            
        case '1': /* Send HTTP POST request to set LED state on HTTP Proxy Server */
            HpsSendRequest(CY_BLE_HPS_HTTP_POST, 1u);
            DBG_PRINTF("An HTTP 'POST' request with parameter 'LED ON' was sent \r\n");
            break;
        ...
        case 's':  /* Start scanning */
            ...
            {
                DBG_PRINTF("Cy_BLE_GAPC_StartScan API Error: %xd \r\n", apiResult);
            }
            else
            {
